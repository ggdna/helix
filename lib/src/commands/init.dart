...
## 2. Declare the DNA packages you want

DNA packages are ordinary packages. Declare them, install them, then list
them under `layers`:

```bash
dart pub add dev:dna_dart      # Dart projects
pnpm add -D dna-ts             # TypeScript projects
```

A dependency that is not listed under `layers` is not a layer.
