...
- Consumed by the engine itself: `*.overrides.md`, `*.overrides.json`
  sidecars, plus its own bookkeeping — `dna/_dna.json` (layers and
  hashes) and `dna/_instances.json` (the project files the DNA owns).
  The effective variables are not bookkeeping: they are content and live
  in `dna/_vars.json`.
